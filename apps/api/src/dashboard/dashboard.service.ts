...
export class DashboardService {
  ...
  async summary() {
    ...
    const [allSales, allExpenses, monthSales, monthExpenses, openSales, installmentSales] =
      await Promise.all([
    ...
    const totalRevenue = sum(allSales.map((s) => s.total));
    const totalExpenses = sum(allExpenses.map((e) => e.amount));
    const cashBalance = totalRevenue - totalExpenses;

    const todayIn = sum(allSales.filter((s) => s.createdAt >= todayStart).map((s) => s.total));
